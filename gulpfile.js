...
let path = {
	...
	src: {
		html: [
			`${source_folder}/pages/**/*.{html,pug}`,
			`!${source_folder}/pages/_*.{html,pug}`,
		],
	...
	return gulp
		...
		.pipe(
			cleanCSS({ debug: true }, (details) => {
				console.log(
					`INFO CSS MIN: FROM ${details.name}: ${details.stats.originalSize}KB TO ${details.name}: ${details.stats.minifiedSize}KB`
				);
			}).on("error", console.error)
		)
